...
A practical question in biosecurity surveillance is "How much search effort is required to detect a species?". This can be quantified under controlled conditions where the number and location of target species are known and participants are recruited to see how success rate varies. 
...
pred_line =  expand.grid(search_effort_h_ha = seq(0, 10, length = 100),
                         egg_size = c(10, 30, 100),
                         experience = 0:1) %>%
  mutate(egg_size_c = egg_size - mean(db$egg_size)) %>%
  mutate(pred = 1 - exp(-((pars['a'] + pars['b']*egg_size_c + pars['c']*experience) * search_effort_h_ha))) 
ggplot(pred_line, aes(search_effort_h_ha, pred, colour = factor(egg_size_c), linetype = factor(experience))) +
  geom_line() +
...
Expressing the equation in terms of the odds ratio help interpretabilty. 
...
pred_line =  expand.grid(search_effort_h_ha = seq(0, 10, length = 100),
                         egg_size = c(10, 30, 100),
                         egg_size_c = c(10, 30, 100) - mean(db$egg_size),
                         experience = 0:1) %>%
  mutate(log_search_effort_h_ha = log(search_effort_h_ha)) %>%
  mutate(pred = inv_logit(
    pars['a'] + pars['b']*egg_size_c +
      pars['c']*experience + pars['d']*log_search_effort_h_ha)) 
