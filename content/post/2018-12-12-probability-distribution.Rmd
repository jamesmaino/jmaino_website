...
How many probability distributions we can generate by imagining simple natural processes? In this post I use a simple binomial random number generator to produce different random variables with a variety of distributions. Using built in probability densities functions in R, I show how the simulated data (plot bars) approach the exact density (plot lines) and provide an intuitive interpretation of model parameters of commonly encountered distributions. 

## A biological example  
"Nothing in Biology Makes Sense Except in the Light of Evolution"  - Theodosius Dobzhansky, 1973

Nucleotides form the molecular basis of the genetic code, the order of which is approximately random (to the human eye). In the following, we will imagine reading a strand of random DNA.
...
Counting the number of non-A letters until the see the first A gives us a geometric distribution with $p = 1/4$.

```{r}
samples = replicate(n = 10000, which(rbinom(n = 10, size = 1, prob = 0.25) == 1)[1] - 1)
...
```{r, warning=F}
samples = replicate(n = 10000, which(cumsum(rbinom(n = 20, size = 1, prob = 0.25)) == 2)[1] - 2)
...
Counting the number of Tryptophan codons in 1000 codons can be represented as a binomial trial with $n = 1000$ and $p = (1/4)^3$


```{r}
samples = replicate(n = 10000, sum(rbinom(n = 1000, size = 3, prob = 0.25) == 3))
...
p5 = ggplot()+ 
  ...
  # aesthetics
  ggtitle('Big binomial distribution') +
...
However, this distribution is also approximated by a Poisson distibution with $\gamma = np = 1000 (1/4)^3 $ (shown as green dots).
...
The approximation improves as $n \to \infty$ and $p \to 0$. For example, a Poisson distribution would be a poor approximation in the first example of counting A's in a single random DNA letter. We cannot have more than one A in a single letter as Poisson predicts!

```{r}

p1 + geom_point(aes(0:5 , dpois(0:5, 1 * 1/2) * 10000), col = "green")
...
```{r}
samples = replicate(n = 10000, which(rbinom(n = 1000, size = 3, prob = 0.25) ==3)[1] )/1000
...
The time it takes to for three trytophan reads is gamma distributed with $r = 3$.

```{r}
samples1 = replicate(n = 10000, which(rbinom(n = 1000, size = 3, prob = 0.25) == 3)[1] )/1000
...
Suppose we add more and more random of these random variables together. What we find is that the distibution approaches the famous normal distribution. Indeed, normal distributions are so ubiquitous in nature precisely because they arise as the resulting of adding random variables. 

Note that below I truncate the search for tryptophans at 500 ms otherwise it takes a while to simulate the random variable using `rbinom`. In addition, `r` should be much larger than 32 for the simulated data to better approach the normal distribution.

```{r}
r = 32
gamma = 16
nsamples = replicate(n = 10000 * r, which(rbinom(n = 500, size = 3, prob = 0.25) == 3)[1] )/1000
samples = apply(array(nsamples, dim = c(10000, r)), 1, sum)
sum(is.na(samples))/length(samples) # check that not too many trials were truncated
...
It's  nice to think about how simple processes can generate such a variety of probability distribution functions. A great graphic was produced by by Ehsan azhdari showing the relationships between probability distributions. 
